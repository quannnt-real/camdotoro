...
Ảnh trong `v2.html` và 2 ảnh accent của `index.html` dùng **ảnh stock Pexels** (giấy phép Pexels: miễn phí cho mục đích thương mại, không bắt buộc ghi nguồn), chọn đúng chủ thể (ô tô, xe máy, nhà đất, vàng, bắt tay). Vẫn nên **thay bằng ảnh thật của cửa hàng** để tăng độ tin cậy. **Trước khi chạy thật phải thay bằng ảnh của chính cửa hàng / tài sản (đúng bản quyền)**; ảnh thật còn giúp tăng độ tin cậy. Mỗi ảnh đã có comment `ẢNH DEMO` để dễ tìm và thay.
